...
def get_base_data(html):
    ...
    baseData['storename'] = html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_ctl58_lblShisetsuName').text if len(
        html.find_elements(By.ID, 'ctl00_ContentPlaceHolderContents_ctl58_lblShisetsuName')) > 0 else '-'
    print(baseData)
    baseData['updated_at'] = html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate').text.split('最終報告日')[1] if len(html.find_elements(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate')) > 0 else '-'
    baseData['address'] = html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_ctl58_lblAddress').text.replace('\u3000', ' ') if len(
        html.find_elements(By.ID, 'ctl00_ContentPlaceHolderContents_ctl58_lblAddress')) > 0 else '-'
    try:
...
def main():
    ...
    for line in f:
        ...
        if baseData:
            service_tab = driver.find_element(
                By.NAME, 'ctl00$ContentPlaceHolderContents$btnDetail03')
            service_tab.click()
